...
#define KOMODO_DEX_BLAST (iter/4)  // define as iter to make it have 10 different priorities, as 0 to blast diff 0
...
int32_t komodo_DEXpurge(uint32_t cutoff)
{
    ...
    if ( n != 0 || (modval % 60) == 0 )//totalhash != prevtotalhash )
    {
        ...
        for (i=13; i>=0; i--)
            fprintf(stderr,"%.0f ",histo[i]);//1000.*histo[i]/(total+1)); // expected 1 1 2 5 | 10 10 10 10 10 | 10 9 9 7 5
