                                         ...
                                         unsigned int nBlockHeight)
{
    ...
    // requireGreater means we are looking for the lowest fee such that all higher
    // values pass, so we start at maxbucketindex (highest fee) and look at succesively
    // smaller buckets until we reach failure.  Otherwise, we are looking for the highest
    // fee such that all lower values fail, and we go in the opposite direction.
...
void CBlockPolicyEstimator::Write(CAutoFile& fileout, bool hasPriStats)
{
    fileout << nBestSeenHeight;
    feeStats.Write(fileout);
    if (hasPriStats) priStats.Write(fileout);
}

void CBlockPolicyEstimator::Read(CAutoFile& filein, bool hasPriStats)
{
    int nFileBestSeenHeight;
    filein >> nFileBestSeenHeight;
    feeStats.Read(filein);
    if (hasPriStats) priStats.Read(filein);
