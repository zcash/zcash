...
// pool, we select by highest fee rate, so we might consider
...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const MinerAddress& minerAddress)
{
...

    {
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            CFeeRate feeRate(nTotalIn + nFeeDelta - tx.GetValueOut(), nTxSize);
            ...
            if (porphan)
            {
                ...
                vecPriority.push_back(TxPriority(feeRate, &(mi->GetTx())));
        }
        ...
        {
            // Take highest fee transaction off the priority queue:
