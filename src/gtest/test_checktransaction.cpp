...
TEST(ContextualCheckShieldedInputsTest, BadTxnsInvalidJoinsplitSignature) {
    ...
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, 0, false, [](const Consensus::Params&) { return true; });
    EXPECT_CALL(state, DoS(10, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, 0, false, [](const Consensus::Params&) { return false; });
    // for transactions that have been mined in a block, DoS ban score should
    // always be 100.
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, 0, true, [](const Consensus::Params&) { return true; });
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, 0, true, [](const Consensus::Params&) { return false; });
}
...
TEST(ContextualCheckShieldedInputsTest, JoinsplitSignatureDetectsOldBranchId) {
    ...
    EXPECT_TRUE(ContextualCheckShieldedInputs(
        tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, false,
    ...
    EXPECT_FALSE(ContextualCheckShieldedInputs(
        tx, txdata, state, view, saplingAuth, orchardAuth, consensus, blossomBranchId, false,
        [](const Consensus::Params&) { return false; }));
    ...
    EXPECT_FALSE(ContextualCheckShieldedInputs(
        tx, txdata, state, view, saplingAuth, orchardAuth, consensus, heartwoodBranchId, false,
        [](const Consensus::Params&) { return false; }));
}
...
TEST(ContextualCheckShieldedInputsTest, NonCanonicalEd25519Signature) {
    ...
    // Check that the signature is valid before we add L
    {
        ...
        const PrecomputedTransactionData txdata(tx, allPrevOutputs);
        MockCValidationState state;
        EXPECT_TRUE(ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, true));
    }
    ...
    // mempool (and thus not mined), DoS ban score should be zero, else 10
    EXPECT_CALL(state, DoS(0, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, false, [](const Consensus::Params&) { return true; });
    EXPECT_CALL(state, DoS(10, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, false, [](const Consensus::Params&) { return false; });
    // for transactions that have been mined in a block, DoS ban score should
    // always be 100.
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, true, [](const Consensus::Params&) { return true; });
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-invalid-joinsplit-signature", false, "")).Times(1);
    ContextualCheckShieldedInputs(tx, txdata, state, view, saplingAuth, orchardAuth, consensus, saplingBranchId, true, [](const Consensus::Params&) { return false; });
}
...
TEST(ChecktransactionTests, HeartwoodEnforcesSaplingRulesOnShieldedCoinbase) {
    ...
    CCoinsViewCache view(&baseView);
    EXPECT_TRUE(ContextualCheckShieldedInputs(
        tx, txdata, state, view, saplingAuth, orchardAuth, chainparams.GetConsensus(), heartwoodBranchId, true));
