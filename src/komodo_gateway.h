    ...
    HASH_ITER(hh,PAX,pax,tmp)
    {
        ...
        *script++ = 0xac;
        ...
        else
        {
            ...
            printf(" len.%d vout.%u DEPOSIT %.8f <- pax.%s pending ht %d %d %.8f | ",len,pax->vout,(double)txNew->vout[numvouts].nValue/COIN,symbol,pax->kmdheight,pax->otherheight,dstr(PENDING_KOMODO_TX));
        }
        if ( numvouts++ >= 64 )
            break;
    }
