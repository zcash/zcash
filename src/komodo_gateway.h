...
const char *komodo_opreturn(int32_t height,uint64_t value,uint8_t *opretbuf,int32_t opretlen,uint256 txid,uint16_t vout,char *source)
{
    ...
    else if ( opretbuf[0] == 'D' )
    {
        ...
        if ( opretlen == 38 ) // any KMD tx
        {
            ...
            {
                didstats = 0;
                if ( strcmp("USD",ASSETCHAINS_SYMBOL) == 0 && kmdheight > 214700 && strcmp(base,ASSETCHAINS_SYMBOL) == 0 )
                {
                        ...
                        if ( (pax2= komodo_paxfind(txid,vout,'I')) != 0 )
                        {
                            ...
                            if ( pax2->didstats == 0 )
                            {
                                if ( (basesp= komodo_stateptrget(base)) != 0 )
                                {
                                    ...
                                    pax2->didstats = 1;
                                    if ( 1 && strcmp(base,ASSETCHAINS_SYMBOL) == 0 )
                                        printf("########### %p issueda %s += %.8f kmdheight.%d %.8f other.%d %s\n",basesp,base,dstr(pax2->fiatoshis),pax2->height,dstr(pax2->komodoshis),pax2->otherheight,bits256_str(str,pax2->txid));
                                }
                            }
                        }
                    }
