...
// NU6.3 / Ironwood (MOCK): the Ironwood pool reuses the Orchard commitment-tree type
// (OrchardMerkleFrontier) but is a separate pool with its own anchors.
...
enum ShieldedType: uint8_t
{
    ...
    SAPLING = 0x02,
    ORCHARD = 0x03,
    IRONWOOD = 0x04, // NU6.3 / Ironwood (MOCK): second Orchard-style pool.
};
...
public:
    ...
    //! Retrieve the tree (Ironwood) at a particular anchored root in the chain. MOCK.
...
public:
    ...
    // NU6.3 / Ironwood (MOCK): the Ironwood pool reuses the OrchardMerkleFrontier tree
    // type, so it cannot use the type-dispatched PushAnchor template; push explicitly.
