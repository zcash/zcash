...
const string strMessageMagic = "SodaToken Signed Message:\n";
                      ...
                      libsodatoken::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*psodatokenParams, verifier, tx.joinSplitPubKey)) {
...
void ThreadScriptCheck() {
    RenameThread("sodatoken-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    auto verifier = libsodatoken::ProofVerifier::Strict();
    auto disabledVerifier = libsodatoken::ProofVerifier::Disabled();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In SodaToken this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see SodaToken protocol spec
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libsodatoken::ProofVerifier::Disabled();
