...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    if (fAllowFree)
    {
        ...
        // * If we are relaying we allow transactions up to DEFAULT_BLOCK_SHIELDED_SIZE - 1000
        //   to be considered to fall into this category. We don't want to encourage sending
        //   multiple transactions instead of one big transaction to avoid fees.
        if (nBytes < (DEFAULT_BLOCK_SHIELDED_SIZE - 1000))
            nMinFee = 0;
    }
                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee)
{
...

    {
        ...
        CTxMemPoolEntry entry(tx, nFees, GetTime(), chainActive.Height(), mempool.HasNoInputsOf(tx), fSpendsCoinbase, consensusBranchId);
