    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        BOOST_FOREACH(const uint256 serial, txin.GetZerocoinSerialNumbers()) {
            if (zerocoinSerialNumbers.count(serial)) {
                ...
                return state.DoS(100, error("CheckTransaction() : duplicate serial number %s in tx %s. Already used previously in block. ",
                                            serial.ToString(), tx.GetHash().ToString()),
...
{
    fRejectInsaneFee = false; /* Zerocash disables this check */
    ...
    // Because check inputs is conducted against a dummy coinview, we CANNOT rely on it to do the serial number check
    ...
    if (!tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (tx.vin[i].IsZCPour()) {
                const CAmount contribution = tx.vin[0].GetBtcContributionOfZerocoinTransaction();
