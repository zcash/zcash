...
void CheckHaveAddr(const std::optional<libzcash::PaymentAddress>& addr) {
    ...
    BOOST_CHECK(pwalletMain->ZTXOSelectorForAddress(*addr_of_type, true, false, false).has_value());
}
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_internals)
{
    ...
    {
        auto selector = pwalletMain->ZTXOSelectorForAddress(taddr1, true, false, false).value();
    ...
    {
        auto selector = pwalletMain->ZTXOSelectorForAddress(zaddr1, true, false, false).value();
        WalletTxBuilder builder(*pwalletMain, minRelayTxFee);
