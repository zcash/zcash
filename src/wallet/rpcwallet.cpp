...
UniValue listaddresses(const UniValue& params, bool fHelp)
{
    ...
    {
        std::set<CTxDestination> transparentDests;
        ...
        for (const std::pair<CTxDestination, CAddressBookData>& item : pwalletMain->mapAddressBook) {
            transparentDests.insert(item.first);
        }
        ...
        for (const CTxDestination& dest : transparentDests) {
            UniValue payload(UniValue::VOBJ);
            payload.pushKV("address", keyIO.EncodeDestination(dest));

            auto script = GetScriptForDestination(dest);
            payload.pushKV("watchonly", pwalletMain->HaveWatchOnly(script));
