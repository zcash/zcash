...
#include "wallet/asyncrpcoperation_sendmany_template.h"
...
UniValue z_sendmany_template(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "z_sendmany_template \"fromaddress\" [{\"address\":... ,\"amount\":...},...] ( FIXME ) ( fee )\n"
            "\nSend multiple times. Amounts are decimal numbers with at most 8 digits of precision."
    ...
    // Recipients
    std::vector<z_template::SendManyRecipient> taddrRecipients;
    std::vector<z_template::SendManyRecipient> zaddrRecipients;
    CAmount nTotalOut = 0;
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (isZaddr) {
            zaddrRecipients.push_back( z_template::SendManyRecipient(address, nAmount, memo) );
        } else {
            taddrRecipients.push_back( z_template::SendManyRecipient(address, nAmount, memo) );
        }
    ...
    std::shared_ptr<AsyncRPCQueue> q = getAsyncRPCQueue();
    std::shared_ptr<AsyncRPCOperation> operation( new z_template::AsyncRPCOperation_sendmany_template(builder, contextualTx, fromaddress, taddrRecipients, zaddrRecipients, nMinDepth, nFee, contextInfo, params[2]) );
