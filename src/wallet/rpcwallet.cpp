...
UniValue z_listutxos(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 5)
        throw runtime_error(
            "z_listutxos account ( minconf maxconf asOfHeight )\n"
            "\nReturns the unspent transparent outputs for the account."
            "\nArguments:\n"
            "1. account      (numeric) The account number.\n"
            "2. minconf      (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "3. maxconf      (numeric, optional, default=9999999) The maximum confirmations to filter\n"
            "4. " + asOfHeightMessage(true) +
            "5. includeWatchonly (bool, optional, default=false) Also include watchonly addresses (see 'z_importviewingkey')\n"
            "\nResult\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"txid\" : \"txid\",          (string) the transaction id \n"
            ...
            "]\n"
            "\nExamples:\n"
            "\nThe unspent transparent outputs for account 0\n"
            + HelpExampleCli("z_listutxos", "0") +
            "\nThe unspent transparent outputs for account 1 that have at least 2 confirmations\n"
            + HelpExampleCli("z_listutxos", "1 2") +
            "\nThe unspent transparent outputs for account 1 including watch-only outputs\n"
            + HelpExampleCli("z_listutxos", "1 0 9999999 -1 true")
        );
    ...
    int nMinDepth = 1;
    if (params.size() > 1)
        nMinDepth = parseMinconf(1, params, 1, asOfHeight);
    if (nMinDepth < 0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid minconf, must be >= 0");
    }
    ...
    if (nMaxDepth < 0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid maxconf, must be >= 0");
    }

    if (nMaxDepth < nMinDepth) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid maxconf, must be greater than minconf");
    }

    bool includeWatchonly = false;
    if (params.size() > 4) {
        includeWatchonly = params[4].get_bool();
    }
    ...
    std::set<libzcash::ReceiverType> receiverTypes({libzcash::ReceiverType::P2PKH, libzcash::ReceiverType::P2SH});
    auto selector = pwalletMain->ZTXOSelectorForAccount(account, !includeWatchonly, TransparentCoinbasePolicy::Allow, receiverTypes);
    ...
    auto spendableInputs = pwalletMain->FindSpendableInputs(selector.value(), nMinDepth, asOfHeight);

    UniValue results(UniValue::VARR);
    for (const COutput& out : spendableInputs.utxos) {
        if (out.nDepth < nMinDepth || out.nDepth > nMaxDepth)
            continue;
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "z_listutxos",              &z_listutxos,              false },
