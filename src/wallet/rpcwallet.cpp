...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry, const std::optional<int>& asOfHeight)
{
    ...
    for (const std::pair<const string, string>& item : wtx.mapValue)
...
UniValue listaddresses(const UniValue& params, bool fHelp)
{
    ...
    for (const std::pair<const CTxDestination, CAmount>& item : pwalletMain->GetAddressBalances(std::nullopt)) {
...
extern CScript createmultisig_redeemScript(const UniValue& params);
