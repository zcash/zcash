 ...
 */
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, bool fIncludeCoinBase, boost::optional<size_t> resultLimit) const
{
    vCoins.clear();
    size_t nResults = 0;
    {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    ...
                    (!coinControl || !coinControl->HasSelected() || coinControl->fAllowOtherInputs || coinControl->IsSelected((*it).first, i)))
                    {
                        ...
                        // Only return number of results upto desired limit.
                        if (resultLimit && ++nResults >= *resultLimit) {
                            return;
                        }
                    }
            }
