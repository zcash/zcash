...
CWalletTx GetValidReceive(const libsodatoken::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libsodatoken::Note GetNote(const libsodatoken::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libsodatoken::SpendingKey& sk,
