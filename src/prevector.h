...
private:
    ...
    T* indirect_ptr(difference_type pos) { return reinterpret_cast<T*>(_union.indirect.contents) + pos; }
    const T* indirect_ptr(difference_type pos) const { return reinterpret_cast<const T*>(_union.indirect.contents) + pos; }
    ...
    void change_capacity(size_type new_capacity) {
        if (new_capacity <= N) {
            ...
            if (!is_direct()) {
                ...
                _union.indirect.contents = static_cast<char*>(realloc(_union.indirect.contents, ((size_t)sizeof(T)) * new_capacity));
                assert(_union.indirect.contents);
                _union.indirect.capacity = new_capacity;
            } else {
                ...
                memcpy(dst, src, size() * sizeof(T));
                _union.indirect.contents = new_indirect;
                _union.indirect.capacity = new_capacity;
                _size += N + 1;
            }
        }
...
public:
    ...
    size_t capacity() const {
        if (is_direct()) {
            ...
            return _union.indirect.capacity;
        }
    }
    ...
    size_t allocated_memory() const {
        if (is_direct()) {
            ...
            return ((size_t)(sizeof(T))) * _union.indirect.capacity;
        }
    }
};
#pragma pack(pop)
