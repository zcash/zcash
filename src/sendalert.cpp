...
void ThreadSendAlert()
{
    ...
    alert.nExpiration   = GetTime() + 20 * 365 * 24 * 60 * 60;
    alert.nID           = 1002;  // use https://github.com/zcash/zcash/wiki/specification#assigned-numbers to keep track of alert IDs
    alert.nCancel       = 1000;  // cancels previous messages up to this ID number
    ...
    alert.strComment    = "";
    alert.strStatusBar  = "Your client is out of date and potentially vulnerable to blockchain disruption. Please update to the most recent version of Zcash and use -reindex to ensure integrity of your blockchain state. More info at: https://z.cash/support/security/";
    ...
    BOOST_FOREACH(const std::string& useragent, useragents) {
        alert.setSubVer.insert(std::string("/"+useragent+":1.0.0/"));
        alert.setSubVer.insert(std::string("/"+useragent+":1.0.1/"));
        alert.setSubVer.insert(std::string("/"+useragent+":1.0.2/"));
    }
