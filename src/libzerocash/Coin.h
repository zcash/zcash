...
public:
	 ...
	 */
    Coin(const PublicAddress& addr, uint64_t value);

    Coin(const PublicAddress& addr, uint64_t value,
		 const std::vector<unsigned char>& rho, const std::vector<unsigned char>& r);


	const PublicAddress& getPublicAddress() const;

	const CoinCommitment& getCoinCommitment() const; // fixme this should be private. Is public for tests that don't want to invoke mint.
...
private:
	...
	std::vector<unsigned char> coinValue; // TODO: should be uint64_t
