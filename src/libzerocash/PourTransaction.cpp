/**
 * @file       PourTransaction.cpp
 *
 * @brief      PourTransaction class for the Zerocash library.
 *
...
PourTransaction::PourTransaction(uint16_t version_num, ZerocashParams& params,
								 const MerkleRootType& rt,
								 const Coin& c_1_old, const Coin& c_2_old,
								 const Address& addr_1_old, const Address& addr_2_old,
								 const auth_path& patMAC_1, const auth_path& patMAC_2,
								 const PublicAddress& addr_1_new, const PublicAddress& addr_2_new,
								 uint64_t v_pub, const std::vector<unsigned char>& pubkeyHash,
								 const Coin& c_1_new, const Coin& c_2_new): publicValue(v_size), serialNumber_1(sn_size), serialNumber_2(sn_size), MAC_1(h_size), MAC_2(h_size)
{
	// Set the Pour version
	...
	// Load all of the main elements into bool vectors
	// TODO: add length and sanity checks
    ...
    std::vector<bool> nonce_new_1_bv(rho_size * 8); //rho
...
//pi_pour
    if(this->version > 0){
        pour_proof<default_pp> proofObj = zerocash_pour_ppzksnark_prover<default_pp>(params.getProvingKey(this->version),
                                                                    patMAC_1,
                                                                    patMAC_2,
                                                                    root_bv,
                                                                    addr_pk_new_1_bv,
 	   ...
 	   this->zkSNARK = std::string(1235,'A'); // dummy sized string for snark
    }
...
bool PourTransaction::verify(ZerocashParams& params, std::vector<unsigned char> &pubkeyHash, const MerkleRootType &merkleRoot) const
{
	if(this->version == 0){
		return true;
	}

	// Deserialize the zkSNARK proof into a pour_proof object
    pour_proof<default_pp> proof_SNARK;
    std::stringstream ss;
    ss.str(this->zkSNARK);
    ss >> proof_SNARK;

	// Verify all the vector lengths
    ...
    bool snark_result = zerocash_pour_ppzksnark_verifier<default_pp>(params.getVerificationKey(this->version), root_bv, sn_old_1_bv, sn_old_2_bv, cm_new_1_bv, cm_new_2_bv, val_pub_bv, h_S_bv, MAC_1_bv, MAC_2_bv, proof_SNARK);
...
const CoinCommitmentValue& PourTransaction::getNewCoinCommitmentValue1() const{
	return this->cm_1.getCommitmentValue(); // we return the underlying vector
	// because the other side in bitcoind  can simply call
	// uint256 hash = pour.getNewCoinCommitmentValue1()
	// and this will auto convert.
}
