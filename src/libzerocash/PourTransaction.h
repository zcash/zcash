...
public:
     ...
     */
    PourTransaction( uint16_t version_num, ZerocashParams& params, const MerkleRootType& roott, const Coin& c_1_old, const Coin& c_2_old,
    		const Address& addr_1_old, const Address& addr_2_old,
    		const auth_path& path_1, const auth_path& path_2,
    		const PublicAddress& addr_1_new, const PublicAddress& addr_2_new,
    		uint64_t v_pub, const std::vector<unsigned char>& pubkeyHash,
            const Coin& c_1_new, const Coin& c_2_new);
     ...
     */
	bool verify(ZerocashParams& params, std::vector<unsigned char> &pubkeyHash, const MerkleRootType &merkleRoot) const;
...
private:
    ...
    //std::vector<unsigned char>	pk_sig;				// one-time use ECDSA public key for this Pour
