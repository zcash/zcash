...
namespace libzerocash {
    ...
    bool
    IncrementalMerkleTree::getWitness(const std::vector<bool> &index, auth_path &witness) {
    ...
    IncrementalMerkleNode::~IncrementalMerkleNode()
    {
        ...
        // TODO: Do we need to destroy the sha256 context?
    }
    ...
    bool
    IncrementalMerkleNode::getWitness(const std::vector<bool> &index, auth_path &witness)
    {
        bool result = false;
        ...
        if (index.at(nodeDepth) == false) {
			...
			if (this->right == NULL) {
				witness.at(nodeDepth).aux_digest.resize(SHA256_BLOCK_SIZE * 8);
				std::fill (witness.at(nodeDepth).aux_digest.begin(), witness.at(nodeDepth).aux_digest.end(), false);
			} else {
				this->right->getValue(witness.at(nodeDepth).aux_digest);
				printVectorAsHex(witness.at(nodeDepth).aux_digest);
			}
			
            witness.at(nodeDepth).computed_is_right = false;
        ...
        if (index.at(nodeDepth) == true) {
            this->left->getValue(witness.at(nodeDepth).aux_digest);
            witness.at(nodeDepth).computed_is_right = true;
    ...
    IncrementalMerkleNode::updateHashValue()
    {
		...
		//std::cout << "hash at level " << this->nodeDepth << ": ";
		//printVectorAsHex(this->value);
		//std::cout << std::endl;
    }
    ...
    IncrementalMerkleNode::fromCompactRepresentation(IncrementalMerkleTreeCompact &rep, uint32_t pos)
    {
        ...
        if (rep.hashList.at(this->nodeDepth) == true) {
			...
			//std::cout << "Added an output:";
			//printBytesVectorAsHex(rep.hashVec[pos]);
			//std::cout << std::endl;
