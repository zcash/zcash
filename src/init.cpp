...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-printtoconsole", _("Send trace/debug info to console instead of debug log"));
    if (showDebug)
    {
        strUsage += HelpMessageOpt("-printpriority", strprintf("Log transaction priority and fee per kB when mining blocks (default: %u)", DEFAULT_PRINTPRIORITY));
    }
    // strUsage += HelpMessageOpt("-shrinkdebugfile", _("Shrink debug log on client startup (default: 1 when no -debug)"));
