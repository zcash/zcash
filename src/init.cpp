...
ZCJoinSplit* psodatokenParams = NULL;
...
void Shutdown()
{
    ...
    RenameThread("sodatoken-shutoff");
    ...
    pwalletMain = NULL;
#endif
    delete psodatokenParams;
    psodatokenParams = NULL;
...
{
    RenameThread("sodatoken-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
static void ZC_LoadParams()
{
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path))) {
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the SodaToken network parameters in the following directory:\n"
              "%s\n"
    ...
    psodatokenParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SodaToken version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SodaToken is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SodaToken is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SodaToken is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize SodaToken circuit parameters
    ZC_LoadParams();
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SodaToken") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SodaToken to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("SodaToken was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild SodaToken with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("SodaToken was not built with wallet support. Set -mineraddress, or rebuild SodaToken with wallet support."));
    }
 #endif // !ENABLE_WALLET
