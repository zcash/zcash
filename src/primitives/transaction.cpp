...
template<std::size_t N>
boost::array<uint256, N> unsigned_char_to_uint256_array(const boost::array<std::vector<unsigned char>, N>& in) {
            ...
            CAmount vpub_new) : scriptSig(), scriptPubKey(scriptPubKey), vpub_old(vpub_old), vpub_new(vpub_new), anchor(anchor)
{
    ...
    serials = unsigned_char_to_uint256_array(serials_bv);
    commitments = unsigned_char_to_uint256_array(commitments_bv);
    macs = unsigned_char_to_uint256_array(macs_bv);
