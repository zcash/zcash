...
std::unique_ptr<BundleValidityCache> NewBundleValidityCache(rust::Str kind, size_t nMaxCacheSize)
{
    ...
    LogPrintf(
        "Using %zu MiB out of %zu requested for %s bundle cache, able to store %zu elements\n",
        (nElems * sizeof(BundleCacheEntry)) >> 20,
        nMaxCacheSize >> 20,
        kind,
        nElems);
    return cache;
...
template bool libzcash::BundleValidityCache::contains(
