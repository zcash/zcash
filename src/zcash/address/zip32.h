...
public:
    ...
    friend bool operator==(const HDSeed& a, const HDSeed& b) { return a.seed == b.seed; }

    friend bool operator!=(const HDSeed& a, const HDSeed& b) { return !(a == b); }
};
...
public:
    ...
    DecryptDiversifier(const SaplingPaymentAddress& addr) const
    {
    ...
    friend inline bool operator==(
        const SaplingDiversifiableFullViewingKey& a,
        const SaplingDiversifiableFullViewingKey& b)
    {
...
struct SaplingExtendedSpendingKey {
    ...
    static std::pair<SaplingExtendedSpendingKey, HDKeyPath>
    ForAccount(const HDSeed& seed, uint32_t bip44CoinType, libzcash::AccountId accountId);
    static std::pair<SaplingExtendedSpendingKey, HDKeyPath>
    Legacy(const HDSeed& seed, uint32_t bip44CoinType, uint32_t addressIndex);
