...
public:
     ...
     * to check that the generated seed can produce valid transparent and unified addresses at
     * account numbers 0x7FFFFFFF and 0x00 respectively.
     */
    static MnemonicSeed
    Random(uint32_t bip44CoinType, Language language = English, size_t entropyLen = 32);

    static MnemonicSeed
    FromLegacySeed(const HDSeed& legacySeed, uint32_t bip44CoinType, Language language = English);
    ...
    const SecureString& GetMnemonic() const { return mnemonic; }
    ...
    friend bool operator!=(const MnemonicSeed& a, const MnemonicSeed& b) { return !(a == b); }
};
