...
// Alright - using this to test initial p2sh consensus changes
// adapted z_sendmany to produce unsigned t->z transactions which can then be handled by external code to add sigs
// this could be cleaned up quite a bit if used in production
boost::optional<CTransaction> TransactionBuilder::BuildWithoutSig()
{
    //
    // Consistency checks
    //
