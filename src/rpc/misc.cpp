...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric, optional) the total " + CURRENCY_UNIT + " balance of the wallet, if wallet functionality is enabled\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given " PACKAGE_NAME " address.\n"
            "\nArguments:\n"
            "1. \"zcashaddress\"     (string, required) The " PACKAGE_NAME " address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"zcashaddress\",   (string) The " PACKAGE_NAME " address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are " PACKAGE_NAME " addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) " PACKAGE_NAME " address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"zcashaddress\"    (string, required) The " PACKAGE_NAME " address to use for the signature.\n"
