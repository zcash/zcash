...
bool CheckProofOfWork(int32_t height,uint8_t *pubkey33,uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    if ( height > 34000 ) // 0 -> non-special notary
    {
        ...
        if ( notaryid >= 0 && (height <= 90065 || (height % KOMODO_ELECTION_GAP) > 64) )
        {
            //if ( special2 == -2 )
            //    printf("height.%d special2.%d special.%d\n",height,special2,special);
            if ( height <= 90065 || (height % KOMODO_ELECTION_GAP) == 0 || (height < 80000 && (special != 0 || special2 > 0)) || (height >= 80000 && special2 > 0) )
            {
