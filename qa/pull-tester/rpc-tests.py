...
def run_tests(test_list, src_dir, build_dir, exeext, jobs=1, enable_coverage=False, args=[]):
    ...
    if len(test_list) > 1 and jobs > 1 and ("SHARD_SCRIPT" not in os.environ or os.environ["SHARD_SCRIPT"] != "true"):
        # Populate cache
...
class RPCTestHandler:
    ...
    def get_next(self):
        while self.num_running < self.num_jobs and self.test_list:
            ...
            self.jobs.append((t,
                              time.time(),
                              subprocess.Popen((["shard.py"] if os.environ["SHARD_SCRIPT"] == "true" else []) + (self.tests_dir + t).split() + self.flags + port_seed,
