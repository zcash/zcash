...
|       Skip the chain supply consistency check on blocks where chain supply
|       tracking fields are missing, instead of aborting the node
|       (regtest-only). Needed only for tests that load a regtest cache
|       generated by a zcashd version before v5.4.0
|
|  -regtestchainsupplycheckpoint
|       Enable a hardcoded chain supply checkpoint at the regtest cache tip
