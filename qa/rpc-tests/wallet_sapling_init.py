...
# Test cases of wallet initialization in rollback scenarios
class SaplingWalletInitTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 4
    ...
    def run_test(self):
        ...
        ua0 = self.nodes[0].z_getaddressforaccount(acct0, ['sapling'])['address']
        ...
        ua1 = self.nodes[1].z_getaddressforaccount(acct1, ['sapling'])['address']
        ...
        assert_equal(
                {'pools': {'sapling': {'valueZat': 9_0000_0000}}, 'minimum_confirmations': 1},
                self.nodes[1].z_getbalanceforaccount(acct1))

        assert_equal(
                {'pools': {'sapling': {'valueZat': 1_0000_0000}}, 'minimum_confirmations': 1},
        ...
        # Restart the network, still split; the split here is not necessary to reproduce
        ...
        ua0new = self.nodes[0].z_getaddressforaccount(acct0, ['sapling'])['address']
        ...
        assert_equal(
                {'pools': {'sapling': {'valueZat': 8_3000_0000}}, 'minimum_confirmations': 1},
                self.nodes[1].z_getbalanceforaccount(acct1))


if __name__ == '__main__':
    SaplingWalletInitTest().main()
