...
class TxExpiringSoonTest(ZcashTestFramework):
    ...
    def verify_inv(self, testnode, tx):
        ...
        with mininode_lock:
            ...
            self.assertEqual(len(msg.inv), 1)
            self.assertEqual(tx.sha256, msg.inv[0].hash)
    ...
    def verify_last_tx(self, testnode, tx):
        ...
        with mininode_lock:
            ...
            incoming_tx.rehash()
            self.assertEqual(tx.sha256, incoming_tx.sha256)
    ...
    def run_test(self):
        ...
        self.assertEqual(self.nodes[1].getblockcount(), 200)
        self.assertEqual(self.nodes[2].getblockcount(), 0)
        ...
        self.assertEqual([], self.nodes[0].getrawmempool())
        self.assertEqual([], self.nodes[1].getrawmempool())
        self.assertEqual([], self.nodes[2].getrawmempool())
        ...
        self.assertEqual([tx2.hash], self.nodes[0].getrawmempool())
        self.assertEqual([tx2.hash], self.nodes[1].getrawmempool())
        # node 2 is isolated
        self.assertEqual([], self.nodes[2].getrawmempool())
        ...
        self.assertEqual(self.nodes[0].getblockcount(), 200)
        self.assertEqual(self.nodes[1].getblockcount(), 200)
        self.assertEqual(self.nodes[2].getblockcount(), 201)
        ...
        self.assertEqual(self.nodes[0].getblockcount(), 201)
        self.assertEqual(self.nodes[1].getblockcount(), 201)
        self.assertEqual(self.nodes[2].getblockcount(), 201)
        ...
        # Verify contents of mempool
        self.assertEqual({tx2.hash, tx3.hash}, set(self.nodes[0].getrawmempool()))
        self.assertEqual({tx2.hash, tx3.hash}, set(self.nodes[1].getrawmempool()))
        self.assertEqual({tx3.hash}, set(self.nodes[2].getrawmempool()))

        # Verify banscore for nodes are still zero
        self.assertEqual(0, sum(peer["banscore"] for peer in self.nodes[0].getpeerinfo()))
        self.assertEqual(0, sum(peer["banscore"] for peer in self.nodes[2].getpeerinfo()))
