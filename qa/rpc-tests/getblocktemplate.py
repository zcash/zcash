...
class GetBlockTemplateTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Test 3: Get coinbasevalue if requested
        tmpl = node.getblocktemplate({'capabilities': ['coinbasevalue']})
        assert('coinbasetxn' not in tmpl)
        assert('coinbasevalue' in tmpl)

        # Test 4: coinbasevalue is preferred if supported
        ...
        tmpl = node.getblocktemplate()
        assert('reserved' in tmpl)
        assert(len(tmpl['reserved']) == 64)
        assert(len(tmpl['noncerange']) == 128)

        # Test 6: coinbasetxn checks
        assert('foundersreward' in tmpl['coinbasetxn'])
        assert(tmpl['coinbasetxn']['required'])

        # Test 7: coinbasevalue checks
        tmpl = node.getblocktemplate({'capabilities': ['coinbasevalue']})
        assert('coinbaseaux' in tmpl)
        assert('coinbasefrvalue' in tmpl)
        assert('coinbasefrscript' in tmpl)
        assert(tmpl['coinbasevalue'] >= 4*tmpl['coinbasefrvalue'])
