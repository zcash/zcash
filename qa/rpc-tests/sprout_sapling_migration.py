...
class SproutSaplingMigration(ZcashTestFramework):
    ...
    def setup_nodes(self):
        ...
        self.assertEqual(3, len(extra_args[0]))
        self.assertEqual(0, len(extra_args[1]))
    ...
    def run_migration_test(self, node, sproutAddr, saplingAddr, target_height):
        ...
        self.assertEqual(1, len(operationstatus), "num async operations at 495  % 500")
        self.assertEqual('saplingmigration', operationstatus[0]['method'])
        self.assertEqual(target_height, operationstatus[0]['target_height'])

        result = wait_and_assert_operationid_status_result(node, operationstatus[0]['id'])
        print("result: {}".format(result))
        self.assertEqual('saplingmigration', result['method'])
        self.assertEqual(target_height, result['target_height'])
        self.assertEqual(1, result['result']['num_tx_created'])
        self.assertEqual(1, len(result['result']['migration_txids']))
        self.assertTrue(Decimal(result['result']['amount_migrated']) > Decimal('0'))

        self.assertEqual(0, len(node.getrawmempool()), "mempool size at 495 % 500")

        node.generate(3)
        self.sync_all()

        # At 498 % 500 the mempool will be empty and no funds will have moved
        self.assertEqual(0, len(node.getrawmempool()), "mempool size at 498 % 500")
        self.assertEqual(node.z_getbalance(sproutAddr), Decimal('10'))
        self.assertEqual(node.z_getbalance(saplingAddr), Decimal('0'))
        ...
        self.assertEqual(1, len(mempool), "mempool size at 499 % 500")
        self.assertEqual(node.z_getbalance(sproutAddr), Decimal('0'))
        self.assertEqual(node.z_getbalance(saplingAddr), Decimal('0'))
        self.assertTrue(node.z_getbalance(saplingAddr, 0) > Decimal('0'), "Unconfirmed sapling balance at 499 % 500")
        # Check that unmigrated amount + unfinalized = starting balance - fee
        status = node.z_getmigrationstatus()
        print("status: {}".format(status))
        self.assertEqual(Decimal('9.9999'), Decimal(status['unmigrated_amount']) + Decimal(status['unfinalized_migrated_amount']))

        # The transaction in the mempool should be the one listed in migration_txids,
        # and it should expire at the next 450 % 500.
        self.assertEqual(1, len(status['migration_txids']))
        txid = status['migration_txids'][0]
        self.assertEqual(txid, mempool[0])
        tx = node.getrawtransaction(txid, 1)
        self.assertEqual(target_height + 450, tx['expiryheight'])
        ...
        self.assertTrue(sprout_balance < Decimal('10'), "Should have less Sprout funds")
        ...
        self.assertEqual(sapling_balance, Decimal(status['finalized_migrated_amount']))
