...
def collect_chain_info(rpc: ZcashRPC) -> dict:
    ...
    Required fields are read strictly (no defaults): a missing block height,
    best block hash, or chain name means zcashd returned something unexpected,
    and we would rather fail the build loudly than record placeholder values in
    the manifest. ``size_on_disk`` is genuinely version-dependent (absent on the
    older zcashd builds used in phases 0-3), so it is included only when present
    ...
    if "size_on_disk" in info:
        chain_info["size_on_disk"] = info["size_on_disk"]
    return chain_info
...
def get_zcashd_commit(rpc: ZcashRPC) -> str:
    ...
    real problem with the node.
    """
    info = rpc.call("getnetworkinfo")
    return info["subversion"]
