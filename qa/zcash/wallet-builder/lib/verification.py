...
def _verify_address_diversity(rpc: ZcashRPC) -> list[VerificationError]:
    ...
    # Check transparent presence. getaddressesbyaccount("") is a legacy
    # Bitcoin RPC and does not return addresses on modern zcashd wallets,
    # so we also fall back to listunspent (any transparent UTXO is proof
    # the wallet holds a transparent address).
    try:
    ...
    if "transparent" not in found_pools:
        try:
            utxos = rpc.listunspent(0)
            if utxos:
                found_pools.add("transparent")
                logger.info("  Found %d transparent UTXOs", len(utxos))
        except Exception:
            pass
