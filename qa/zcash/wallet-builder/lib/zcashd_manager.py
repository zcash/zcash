...
def checkpoint_wallet(phase_index: int, datadir: str = PRIMARY_DATADIR) -> str | None:
    """
...
def restore_wallet(checkpoint_path: str, datadir: str = PRIMARY_DATADIR) -> None:
    """
...
def switch_version(from_phase: int, to_phase: int,
    ...
    This is the core version-transition logic. It:
      ...
      3. Starts the new version (no -reindex, no -rescan; see module docstring)
    ...
    # 4. Wait for the new version to be ready. If readiness probing fails,
    # stop the new process so we don't leak a stranded zcashd holding the
    # datadir lock and the RPC port.
    new_rpc = ZcashRPC(rpcport=RPC_PORT_PRIMARY)
    try:
        new_rpc.wait_for_ready(timeout=180)
    except Exception:
        stop_zcashd(rpc=new_rpc, process=new_process)
        raise
...
def verify_version(rpc: ZcashRPC, expected_version: str) -> str | None:
    """
