...
class TxType(Enum):
    """The zcashd RPC used to create a recorded transaction."""
    SHIELD_COINBASE = "z_shieldcoinbase"
...
class TransactionRecord:
    ...
    tx_type: TxType           # Which RPC created the transaction
...
def shield_coinbase(rpc: ZcashRPC, to_address: str, to_pool: str,
    ...
    if policy is not None:
        # Override fee for v5+ to cover ZIP 317 action costs (see constant docs).
        fee = V5_CONVENTIONAL_FEE
    ...
    while remaining > 0:
        ...
        records.append(TransactionRecord(
            txid=txid,
            mined_height=height,
            tx_type=TxType.SHIELD_COINBASE,
            from_pool=Pool.TRANSPARENT.value,
            to_pool=to_pool,
            from_address="*",
...
def send_between_pools(rpc: ZcashRPC, from_address: str, to_address: str,
    ...
    if from_pool == Pool.TRANSPARENT.value and to_pool == Pool.TRANSPARENT.value:
        ...
        if policy is not None:
            ...
            # See V5_CONVENTIONAL_FEE for the ZIP 317 fee-cap rationale.
            v5_fee = V5_CONVENTIONAL_FEE
            actual_fee = v5_fee
    ...
    record = TransactionRecord(
        txid=txid,
        mined_height=height,
        tx_type=TxType.Z_SENDMANY if from_pool != Pool.TRANSPARENT.value or to_pool != Pool.TRANSPARENT.value else TxType.SENDTOADDRESS,
        from_pool=from_pool,
        to_pool=to_pool,
...
def mine_to_height(rpc: ZcashRPC, target_height: int) -> None:
    """
