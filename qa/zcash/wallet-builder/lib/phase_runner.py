...
def _export_wallet(rpc: ZcashRPC, phase_index: int) -> None:
    """Export the full wallet for reference (z_exportwallet)."""
    nu = NETWORK_UPGRADES[phase_index]
    # zcashd's z_exportwallet rejects any non-alphanumeric character in the
    # filename, so strip them from nu.id (e.g. "nu6_1" -> "nu61").
    nu_id_clean = "".join(c for c in nu.id if c.isalnum())
    filename = f"walletexportphase{phase_index:02d}{nu_id_clean}"
...
def _package_final_artifacts() -> None:
    """
