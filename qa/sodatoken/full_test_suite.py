#!/usr/bin/env python2
#
# Execute all of the automated tests related to SodaToken.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/sodatoken/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/sodatoken/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/sodatokend')
    ret &= test_rpath_runpath('src/sodatoken-cli')
    ret &= test_rpath_runpath('src/sodatoken-gtest')
    ret &= test_rpath_runpath('src/sodatoken-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/sodatoken/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/sodatokend')
    ret &= test_fortify_source('src/sodatoken-cli')
    ret &= test_fortify_source('src/sodatoken-gtest')
    ret &= test_fortify_source('src/sodatoken-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/sodatoken/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/sodatoken-gtest')],
