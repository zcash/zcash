...
Backing up your SodaToken private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported SodaToken Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`sodatoken.conf` located in the data directory which is `~/.sodatoken/` unless it's been overridden with `datadir=` setting):
...
Note that sodatokend will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`sodatoken-cli backupwallet <nameofbackup>`.
...
If you generate a new SodaToken address, it will not be reflected in the backup file.
...
`sodatoken-cli z_exportwallet <nameofbackup>`
...
`sodatoken-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`sodatoken-cli z_exportkey <z-address>`
...
`sodatoken-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`sodatoken-cli z_importkey <z-priv-key>`
...
`sodatoken-cli help importprivkey`
