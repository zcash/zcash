...
Jack Grigg (22):
      ...
      Instruct users to run dwcash-fetch-params if network params aren't available
      ...
      Throw an error if dwcash.conf is missing
      Show a friendly message explaining why dwcashd needs a dwcash.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty dwcash.conf during performance measurements
      Create empty dwcash.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from dwcash.conf to avoid problems with command line tools
      Closes #1097 so dwcash-cli now displays license info like dwcashd.
