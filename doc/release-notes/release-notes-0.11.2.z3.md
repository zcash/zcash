...
Nathan Wilcox (4):
      ...
      Add a sodatoken-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the sodatoken-gtest run; invoke with make sodatoken-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to sodatoken-gtest.
      ...
      Small nit fixes
      Run `sodatoken-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libsodatoken` SodaToken protocol API and crypto constructions surrounding the zkSNARK circuit.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to sodatoken.conf and sodatokend.pid in qa/ and src/
