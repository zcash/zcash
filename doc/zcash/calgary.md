...
In this alternative, we use bitflags in the `nVersion` field to indicate the presence of Zcash fields at the end. This would allow us to track upstream CTransaction changes seamlessly, *but* would indefinitely require that upstream `nVersion` bits are available to us for this purpose. Additionally, the absence of these bitflags would conflict in purpose with an empty vector of `ZCOperation` structures as outlined later.

#### ZCOperation

After `zc_nVersion` we add another field to the end of a `CTransaction`, a vector of `ZCOperation` structures called `zc_vop`.
This vector is merely empty in the case of traditional transactions.
The first operation in `zc_vop` may reference the commitment tree root produced by any previous block.
...
The structure of a `ZCOperation` is as follows:
...
* A Zcash-specific versioning field can be added, along with upstream interaction semantics.
