...
during the *configure* step of building animecoind:
...
Launch animecoind like this:

    $ animecoind -amqppubhashtx=amqp://127.0.0.1:5672
...
These options can also be provided in animecoin.conf.
...
From the perspective of animecoind, the local end of an AMQP link is write-only.

No information is broadcast that wasn't already received from the public
P2P network.

No authentication or authorization is done on peers that animecoind connects
...
At present, animecoind does not try to resend a notification if there was
...
Currently, animecoind appends an up-counting sequence number to each notification
